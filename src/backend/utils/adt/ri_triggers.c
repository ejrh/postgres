...
ri_set(TriggerData *trigdata, bool is_set_null)
{
	...
	else
	{
		 ...
		 * already implemented in the NO ACTION trigger, so just run it.
		 * (This recheck is only needed in the SET DEFAULT case, since CASCADE
		 * would remove such rows in case of a DELETE operation or would
		 * change the FK key values in case of an UPDATE, while SET NULL is
